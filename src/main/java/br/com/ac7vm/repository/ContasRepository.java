...
public interface ContasRepository extends JpaRepository<Contas, Integer>{
	...
	@Query("Select c from Contas c where c.instituicao.nome like CONCAT('%', :nome, '%') and c.tipo= :tipo and c.datapagamento is NULL order by c.instituicao.nome")
	Optional<List<Contas>> findAllContasNomePagar(@Param("nome") String nome, @Param("tipo") String tipo);
	
	@Query("Select c from Contas c where c.instituicao.nome like CONCAT('%', :nome, '%') and c.tipo= :tipo and c.valorpago>0 order by c.instituicao.nome")
	...
	@Query("Select c from Contas c where c.instituicao.nome like CONCAT('%', :nome, '%') and c.datavencimento>= :datainicio and c.datavencimento<= :datafinal and c.tipo= :tipo order by c.datavencimento")
	...
	@Query("Select c from Contas c where c.instituicao.nome like CONCAT('%', :nome, '%') and c.datavencimento>= :datainicio and c.datavencimento<= :datafinal and c.tipo= :tipo and c.datapagamento is NULL order by c.datavencimento")
